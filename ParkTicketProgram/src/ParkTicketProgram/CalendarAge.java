...
public class CalendarAge {
	...
	public void ageCalculator() {
		...
		if(idNumbers[6] == 1 || idNumbers[6] == 2) {
			birthYear = (idNumbers[0] * 10) + idNumbers[1] + ConstValue.BEFORE_MILL;
		} else if(idNumbers[6] == 3 || idNumbers[6] == 4) {
			birthYear = (idNumbers[0] * 10) + idNumbers[1] + ConstValue.AFTER_MILL;
		}
		
		int birthMon = (idNumbers[2] * 10) + idNumbers[3];
		int birthDate = (idNumbers[4] * 10) + idNumbers[5];
	
		if(birthMon > ConstValue.NOW_MON || (birthMon == ConstValue.NOW_MON && birthDate > ConstValue.NOW_DATE)) {
			customerAge = ConstValue.NOW_YEAR - birthYear - 1;
		} else {
			customerAge = ConstValue.NOW_YEAR - birthYear;
		}
	}
	...
	public void ticketTypeFinder() {
		...
		ticketType = TYPE_CALENDAR[ConstValue.NOW_MON - 3][ConstValue.NOW_DATE - 1];	
	}
